...
import numpy as np
from . import utilsV3, dynamicsV3
...
def boundary_scores(masks_true, masks_pred, scales):
    ...
    """
    diams = [utilsV3.diameters(lbl)[0] for lbl in masks_true]
    ...
    for j, scale in enumerate(scales):
        for n in range(len(masks_true)):
            ...
            rs, ys, xs = utilsV3.circleMask([int(np.ceil(diam)), int(np.ceil(diam))])
            filt = (rs <= diam).astype(np.float32)
            otrue = utilsV3.masks_to_outlines(masks_true[n])
            otrue = convolve(otrue, filt)
            opred = utilsV3.masks_to_outlines(masks_pred[n])
...
def _label_overlap(x, y):
    ...
    #     x = (utilsV3.format_labels(x)).ravel()
    #     y = (utilsV3.format_labels(y)).ravel()
    x = x.ravel()
    y = y.ravel()
...
def flow_error(maski, dP_net, device=None):
    ...
    dP_masks = dynamicsV3.masks_to_flows(maski, device=device)
