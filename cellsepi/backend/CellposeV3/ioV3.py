...
import re
from .versionV3 import version_str
...
from . import utilsV3, plotV3, transformsV3
...
def remove_model(filename, delete=False):
    ...
    from . import modelsV3
    model_strings = modelsV3.get_user_models()
    if len(model_strings) > 0:
        with open(modelsV3.MODEL_LIST_PATH, "w") as textfile:
            for fname in model_strings:
                textfile.write(fname + "\n")
    else:
        # write empty file
        textfile = open(modelsV3.MODEL_LIST_PATH, "w")
        textfile.close()
    print(f"{filename} removed from custom model list")
    if delete:
        os.remove(os.fspath(modelsV3.MODEL_DIR.joinpath(fname)))
        print("model deleted")
...
def add_model(filename):
    ...
    try:
        shutil.copyfile(filename, os.fspath(modelsV3.MODEL_DIR.joinpath(fname)))
    except shutil.SameFileError:
        pass
    print(f"{filename} copied to models folder {os.fspath(modelsV3.MODEL_DIR)}")
    if fname not in modelsV3.get_user_models():
        with open(modelsV3.MODEL_LIST_PATH, "a") as textfile:
            textfile.write(fname + "\n")
...
def masks_flows_to_seg(images, masks, flows, file_names, diams=30., channels=None,
    ...
    if flows[0].ndim == 3:
        cellprob = (np.clip(transformsV3.normalize99(flows[2]), 0, 1) * 255).astype(
            np.uint8)
        ...
        flowi.append(
            (np.clip(transformsV3.normalize99(flows[2]), 0, 1) * 255).astype(np.uint8))
    ...
    outlines = masks * utilsV3.masks_to_outlines(masks)
...
def save_rois(masks, file_name, multiprocessing=None):
    ...
    Returns:
        None
    """
    outlines = utilsV3.outlines_list(masks, multiprocessing=multiprocessing)
...
def save_masks(images, masks, flows, file_names, png=True, tif=False, channels=[0, 0],
    ...
    if save_mpl and png and MATPLOTLIB and not min(images.shape) > 3:
        ...
        plotV3.show_segmentation(fig, img, masks, flows[0])
    ...
    if masks.ndim < 3 and save_txt:
        check_dir(txtdir)
        outlines = utilsV3.outlines_list(masks)
        outlines_to_text(os.path.join(txtdir, basename), outlines)

    # RGB outline images
    if masks.ndim < 3 and save_outlines:
        check_dir(outlinedir)
        outlines = utilsV3.masks_to_outlines(masks)
        outX, outY = np.nonzero(outlines)
        img0 = transformsV3.normalize99(images)
        if img0.shape[0] < 4:
            img0 = np.transpose(img0, (1, 2, 0))
        if img0.shape[-1] < 3 or img0.ndim < 3:
            img0 = plotV3.image_to_rgb(img0, channels=channels)
        else:
            if img0.max() <= 50.0:
