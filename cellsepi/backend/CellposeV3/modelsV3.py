...
from . import transformsV3, dynamicsV3, utilsV3, plotV3
from .resnet_torchV3 import CPnet
from .coreV3 import assign_device, check_mkl, run_net, run_3D
...
def cache_model_path(basename):
    ...
    if not os.path.exists(cached_file):
        ...
        utilsV3.download_url_to_file(url, cached_file, progress=True)
    return cached_file
...
class CellposeModel():
    ...
    def __init__(self, gpu=False, pretrained_model=False, model_type=None,
        ...
        if backbone == "default":
            ...
            from .segformerV3 import Transformer
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            self.timing = []
            masks, styles, flows = [], [], []
            tqdm_out = utilsV3.TqdmToLogger(models_logger, level=logging.INFO)
            nimg = len(x)
            ...
            # reshape image
            x = transformsV3.convert_image(x, channels, channel_axis=channel_axis,
            ...
            if nimg > 1 and do_normalization and (stitch_threshold or do_3D):
                ...
                x = transformsV3.normalize_img(x, **normalize_params)
            ...
            return masks, [plotV3.dx_to_circ(dP), dP, cellprob], styles
    ...
    def _run_net(self, x, rescale=1.0, resample=True, augment=False, 
        ...
        if do_3D:
            ...
            if rescale != 1.0 or (anisotropy is not None and anisotropy != 1.0):
                ...
                if rescale != 1.0:
                    x = transformsV3.resize_image(x, Ly=int(Ly * rescale),
                                                  Lx=int(Lx*rescale))
                x = transformsV3.resize_image(x.transpose(1, 0, 2, 3),
            ...
            if resample:
                if rescale != 1.0 or Lz != yf.shape[0]:
                    ...
                    if rescale != 1.0:
                        yf = transformsV3.resize_image(yf, Ly=Ly, Lx=Lx)
                    if Lz != yf.shape[0]:
                        yf = transformsV3.resize_image(yf.transpose(1, 0, 2, 3),
            ...
            if resample:
                if rescale != 1.0:
                    yf = transformsV3.resize_image(yf, shape[1], shape[2])
            cellprob = yf[..., 2]
    ...
    def _compute_masks(self, shape, dP, cellprob, flow_threshold=0.4, cellprob_threshold=0.0,
        ...
        tic = time.time()
        if do_3D:
            masks = dynamicsV3.resize_and_compute_masks(
            ...
            for i in iterator:
                ...
                outputs = dynamicsV3.resize_and_compute_masks(
                    dP[:, i], cellprob[i],
            ...
            if stitch_threshold > 0 and nimg > 1:
                ...
                masks = utilsV3.stitch3D(masks, stitch_threshold=stitch_threshold)
                masks = utilsV3.fill_holes_and_remove_small_masks(
...
class SizeModel():
    ...
    def eval(self, x, channels=None, channel_axis=None, normalize=True, invert=False,
        ...
        diam = utilsV3.diameters(masks)[0]
