...
from . import utilsV3, ioV3, transformsV3
...
def dx_to_circ(dP):
    ...
    """
    mag = 255 * np.clip(transformsV3.normalize99(np.sqrt(np.sum(dP**2, axis=0))), 0, 1.)
...
def show_segmentation(fig, img, maski, flowi, channels=[0, 0], file_name=None):
    ...
    outlines = utilsV3.masks_to_outlines(maski)
    ...
    if file_name is not None:
        ...
        ioV3.imsave(save_path + "_overlay.jpg", overlay)
        ioV3.imsave(save_path + "_outlines.jpg", imgout)
        ioV3.imsave(save_path + "_flows.jpg", flowi)
...
def mask_rgb(masks, colors=None):
    ...
    if colors is not None:
        if colors.max() > 1:
            ...
            colors /= 255
        colors = utilsV3.rgb_to_hsv(colors)
    ...
    RGB = (utilsV3.hsv_to_rgb(HSV) * 255).astype(np.uint8)
    return RGB
...
def image_to_rgb(img0, channels=[0, 0]):
    ...
    for i in range(img.shape[-1]):
        if np.ptp(img[:, :, i]) > 0:
            img[:, :, i] = np.clip(transformsV3.normalize99(img[:, :, i]), 0, 1)
...
def outline_view(img0, maski, color=[1, 0, 0], mode="inner"):
    ...
    if SKIMAGE_ENABLED:
        ...
        outlines = utilsV3.masks_to_outlines(maski, mode=mode)
