...
import os
import numpy as np
from . import ioV3, transformsV3, utilsV3, modelsV3, dynamicsV3, metricsV3, resnet_torchV3
from .transformsV3 import normalize_img
...
def _get_batch(inds, data=None, labels=None, files=None, labels_files=None,
    ...
    if data is None:
        lbls = None
        imgs = [ioV3.imread(files[i]) for i in inds]
        ...
        if labels_files is not None:
            lbls = [ioV3.imread(labels_files[i])[1:] for i in inds]
    else:
...
def convert_to_rgb(img):
    if img.ndim == 2:
        ...
        img = transformsV3.normalize99(img)
        img = np.tile(img, (3, 1, 1))
    return img
...
def _reshape_norm(data, channels=None, channel_axis=None, rgb=False,
    ...
    if channels is not None or channel_axis is not None:
        data = [
            transformsV3.convert_image(td, channels=channels, channel_axis=channel_axis)
            for td in data
        ]
        data = [td.transpose(2, 0, 1) for td in data]
    if normalize_params["normalize"]:
        data = [
            transformsV3.normalize_img(td, normalize=normalize_params, axis=0)
            for td in data
        ]
    if rgb:
...
def _reshape_norm_save(files, channels=None, channel_axis=None,
    ...
    for f in trange(files):
        ...
        if channels is not None:
            td = transformsV3.convert_image(td, channels=channels,
                                          channel_axis=channel_axis)
            td = td.transpose(2, 0, 1)
        if normalize_params["normalize"]:
            td = transformsV3.normalize_img(td, normalize=normalize_params, axis=0)
...
def _process_train_test(train_data=None, train_labels=None, train_files=None,
    ...
    if train_data is not None and train_labels is not None:
        ...
        if not load_files:
            ...
            train_data = [ioV3.imread(train_files[i]) for i in trange(nimg)]
            train_labels = [ioV3.imread(train_labels_files[i]) for i in trange(nimg)]
        nimg_test = len(test_files) if test_files is not None else None
        if load_files and nimg_test:
            test_data = [ioV3.imread(test_files[i]) for i in trange(nimg_test)]
            test_labels = [ioV3.imread(test_labels_files[i]) for i in trange(nimg_test)]
    ...
    if train_labels is not None:
        train_labels = dynamicsV3.labels_to_flows(train_labels, files=train_files,
                                                device=device)
        if test_labels is not None:
            test_labels = dynamicsV3.labels_to_flows(test_labels, files=test_files,
                                                   device=device)
    elif compute_flows:
        for k in trange(nimg):
            tl = dynamicsV3.labels_to_flows(ioV3.imread(train_labels_files),
                                          files=train_files, device=device)
        if test_files is not None:
            for k in trange(nimg_test):
                tl = dynamicsV3.labels_to_flows(ioV3.imread(test_labels_files),
    ...
    for k in trange(nimg):
        tl = (train_labels[k][0]
              if train_labels is not None else ioV3.imread(train_labels_files[k])[0])
        diam_train[k], dall = utilsV3.diameters(tl)
        nmasks[k] = len(dall)
    diam_train[diam_train < 5] = 5.
    if test_data is not None:
        diam_test = np.array(
            [utilsV3.diameters(test_labels[k][0])[0] for k in trange(len(test_labels))])
        diam_test[diam_test < 5] = 5.
    elif test_labels_files is not None:
        diam_test = np.array([
            utilsV3.diameters(ioV3.imread(test_labels_files[k])[0])[0]
...
def train_seg(net, train_data=None, train_labels=None, train_files=None,
    ...
    if isinstance(normalize, dict):
        normalize_params = {**modelsV3.normalize_default, **normalize}
    elif not isinstance(normalize, bool):
        raise ValueError("normalize parameter must be a bool or a dict")
    else:
        normalize_params = modelsV3.normalize_default
    ...
    for iepoch in range(n_epochs):
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            rsc = diams / net.diam_mean.item() if rescale else np.ones(
                len(diams), "float32")
            # augmentations
            imgi, lbl = transformsV3.random_rotate_and_resize(imgs, Y=lbls, rescale=rsc,
            ...
            if test_data is not None or test_files is not None:
                ...
                for ibatch in range(0, len(rperm), batch_size):
                    with torch.no_grad():
                        ...
                        rsc = diams / net.diam_mean.item() if rescale else np.ones(
                            len(diams), "float32")
                        imgi, lbl = transformsV3.random_rotate_and_resize(
...
def train_size(net, pretrained_model, train_data=None, train_labels=None,
    ...
    for iepoch in range(n_epochs):
        ...
        for ibatch in range(0, nimg_per_epoch, batch_size):
            ...
            imgi, lbl, scale = transformsV3.random_rotate_and_resize(
    ...
    if nimg_test:
        ...
        for ibatch in range(0, nimg_test_per_epoch, batch_size):
            ...
            imgi = torch.from_numpy(imgi).to(device)
            diamt = np.array([utilsV3.diameters(lbl0[0])[0] for lbl0 in lbl])
