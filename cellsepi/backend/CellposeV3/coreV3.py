...
import fastremap
from . import transformsV3, dynamicsV3, utilsV3, plotV3, metricsV3, resnet_torchV3
...
tqdm_out = utilsV3.TqdmToLogger(core_logger, level=logging.INFO)
...
def run_net(net, imgi, batch_size=8, augment=False, tile_overlap=0.1, bsize=224,
    ...
    if rsz is not None:
        ...
        Lyr, Lxr = Ly0, Lx0
    ypad1, ypad2, xpad1, xpad2 = transformsV3.get_pad_yx(Lyr, Lxr)
    ...
    for k in ziterator:
        ...
        for i, b in enumerate(inds):
            ...
            imgb = transformsV3.resize_image(imgi[b], rsz=rsz) if rsz is not None else imgi[b].copy()
            imgb = np.pad(imgb.transpose(2,0,1), pads, mode="constant")
            IMG, ysub, xsub, Lyt, Lxt = transformsV3.make_tiles(
        ...
        for i, b in enumerate(inds):
            ...
            if augment:
                ...
                y = transformsV3.unaugment_tiles(y)
                y = np.reshape(y, (-1, 3, ly, lx))
            yfi = transformsV3.average_tiles(y, ysub, xsub, Lyt, Lxt)
