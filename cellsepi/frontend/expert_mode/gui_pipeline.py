...
class PipelineGUI(ft.Stack):
    def __init__(self, page: ft.Page):
        ...
        self.show_room_size = len(MODULE_REGISTRY) if len(
            MODULE_REGISTRY) < SHOWROOM_MODULE_COUNT else SHOWROOM_MODULE_COUNT
    ...
    async def load_pipeline(self):
        ...
        for module_dict in self.pipeline_dict["modules"]:
            ...
            self.add_module(module_type=type_map[module_dict["module_name"]], x=module_dict["position"]["x"],
                            y=module_dict["position"]["y"], module_id=module_dict["module_id"], module_dict=module_dict)
    ...
    def check_deletable(self, module: ModuleGUI):
        ...
        return all(not (
                    pipe.target_module.module_id in self.pipeline.run_order or pipe.target_module.module_id == self.pipeline.executing)
                   for pipe in self.pipeline.pipes_out[module.module_id])
    ...
    def expand_connection(self, pipe: Pipe, ports: List[str]):
        ...
        self.lines_gui.update_line(self.modules[pipe.source_module.module_id],
                                   self.modules[pipe.target_module.module_id], pipe.ports)
        self.lines_gui.update_gui()
        self.update_all_port_icons()
    ...
    def refill_show_room(self, module_gui: ModuleGUI, visible: bool = True, index: int = None,
        ...
        """
        new_module_gui = ModuleGUI(self, module_gui.module_type, x=SPACING_X + SHOWROOM_PADDING_X / 2,
                                   y=module_gui.show_offset_y, show_mode=True, visible=visible, index=index,
                                   id_number=show_room_id)
    ...
    def build_show_room(self, page_stack: ft.Stack):
        ...
        def _add_show_room_module(module_type: type, x: float, y: float, visible: bool = True,
                                  show_room_id: int = None):
        ...
        y = SPACING_Y
        self.show_room_container = ft.Container(top=y - SPACING_Y / 2, left=SPACING_X,
                                                width=MODULE_WIDTH + SHOWROOM_PADDING_X,
                                                height=(self.show_room_size * MODULE_HEIGHT) + (
                                                            self.show_room_size * SPACING_Y), bgcolor=MENU_COLOR,
                                                border_radius=ft.border_radius.all(10), blur=10)
    ...
    def add_module(self, module_type: type, x: float = None, y: float = None, module_id: str = None,
    ...
    def check_for_valid(self, module_id: str):
        ...
        if (
                target_module_gui.module_id not in self.pipeline.run_order and target_module_gui.module_id != self.pipeline.executing) or not self.pipeline.running:
            if target_module_gui.module_id != self.source_module:
                ...
                if existing_pipe is None:
                    valid = True
                elif any(port in existing_pipe.ports for port in
                         self.transmitting_ports) or existing_pipe.source_module.module_id != self.source_module or existing_pipe.target_module.module_id != target_module_gui.module_id:
                    valid = False
